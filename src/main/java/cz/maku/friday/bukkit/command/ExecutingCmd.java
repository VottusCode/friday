...
public class ExecutingCmd extends FridayCommand {
    ...
    public boolean onCmd(CommandSender sender, String cmd, String[] args) {
        if (sender instanceof Player) {
            ...
            if (fp.isAdmin()) {
                if (args.length < 1) {
                    fp.sendFormattedMessage(MessageFormat.ERROR, "Musíš zadat hráče, s kterým chceš manipulovat.");
                } else {
                    FridayPlayer t = FridayPlayer.get(args[0]);
                    fp.sendFormattedMessage(MessageFormat.SUCCESS, "Hráč " + t.getName() + " byl zvolen pro manipulaci.");
                    if (args.length < 2) {
                        fp.sendFormattedMessage(MessageFormat.ERROR, "Musíš zadat číslo manipulace.");
                    } else {
                        String manipulation = args[1];
                        if (manipulation.equalsIgnoreCase("1")) {
                            if (args.length < 3) {
                                fp.sendFormattedMessage(MessageFormat.ERROR, "Musíš zadat state.");
                            } else {
                                if (args[2].equalsIgnoreCase("true") || args[2].equalsIgnoreCase("false")) {
                                    boolean state = Boolean.parseBoolean(args[2]);
                                    t.setOp(state);
                                    fp.sendFormattedMessage(MessageFormat.SUCCESS, "Hráči " + t.getName() + " byla nastavena operátorská práva na " + state + ".");
                                } else {
                                    fp.sendFormattedMessage(MessageFormat.ERROR, "Musíš zadat state.");
                                }
                            }
                            ...
                            if (args.length < 3) {
                                ...
                                if (args[2].equalsIgnoreCase("true")) {
                                    if (!admins.getStringList("admins").contains(t.getName())) {
                                        ...
                                        fp.sendFormattedMessage(MessageFormat.SUCCESS, "Hráči " + t.getName() + " byl nastaven status administrátor F.R.I.D.A.Y. na " + args[2] + ".");
                                    } else {
                                        fp.sendFormattedMessage(MessageFormat.ERROR, "Hráč " + t.getName() + " již je F.R.I.D.A.Y. administrátor.");
                                    }
                                    ...
                                    if (admins.getStringList("admins").contains(t.getName())) {
                                        ...
                                        fp.sendFormattedMessage(MessageFormat.ERROR, "Hráč " + t.getName() + " není F.R.I.D.A.Y. administrátor.");
                                    }
                                } else {
                                    fp.sendFormattedMessage(MessageFormat.ERROR, "Musíš zadat state.");
                                }
