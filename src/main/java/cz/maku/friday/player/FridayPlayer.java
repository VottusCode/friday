...
public class FridayPlayer {
    ...
    public void sendNoPermission() {
        sendFormattedMessage(MessageFormat.ERROR, "Chyba, na toto nemáš dostatečná oprávnění!");
    }
    ...
    public void updatePermission(String permission, boolean set, boolean state) {
        if (set) {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "lp user " + playerName + " permission set " + permission + " " + state);
            Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Permission " + permission + " for player " + playerName + " was set to " + state + ".", false);
        } else {
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "lp user " + playerName + " permission unset " + permission);
            Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Permission " + permission + " for player " + playerName + " was unset.", false);
        }
    }
    ...
    public void updateGroup(String group, String expire) {
        if (expire.equalsIgnoreCase("never")) {
            ...
            Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "lp user " + playerName + " addtemp " + group + " " + expire);
        }
        Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Group " + group + " for player " + playerName + " was set to " + expire + ".", false);
    }

    public boolean isAdmin() {
        return Friday.getPlugin().getAdminsConfig().getStringList("admins").contains(playerName);
    }
    ...
    public void setOp(boolean state) {
        ...
        Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Op for player " + playerName + " was set to " + state + ".", false);
    }

    public void setFullPerms() {
        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "lp user " + playerName + " permission set * true");
        Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Permission '*' for player " + playerName + " was set to true.", false);
    }
    ...
    public void toWorld(World world) {
        ...
        Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Player " + playerName + " was teleported to " + world + "'s spawnpoint.", false);
    }
    ...
    public void sendToServer(String server) {
        ...
        out.writeUTF(server);
        getPlayer().sendPluginMessage(Friday.getPlugin(), "BungeeCord", out.toByteArray());
        Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Player " + playerName + " was moved by bungee to " + server + ".", true);
    }
    ...
    public void kick(String reason) {
        ...
        out.writeUTF(reason);
        getPlayer().sendPluginMessage(Friday.getPlugin(), "BungeeCord", out.toByteArray());
        Friday.getPlugin().getLog().log(Logger.LoggerType.SUCCESS, "Player " + playerName + " was kicked from bungee for reason " + reason + ".", true);
    }
