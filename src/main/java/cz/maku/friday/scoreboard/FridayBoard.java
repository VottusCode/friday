package cz.maku.friday.scoreboard;
...
public class FridayBoard {
  ...
  static {
    try {
      if (Reflection.nmsOptionalClass("ScoreboardServer$Action").isPresent()) {
        ...
        Reflection
          .nmsOptionalClass("IScoreboardCriteria$EnumScoreboardHealthDisplay")
          .isPresent()
      ) {
      ...
      Class<?> craftChatMessageClass = Reflection.obcClass(
      ...
      MESSAGE_FROM_STRING =
        craftChatMessageClass.getDeclaredMethod("fromString", String.class);
      ...
      PLAYER_CONNECTION =
        entityPlayerClass.getDeclaredField("playerConnection");
      SEND_PACKET =
        playerConnectionClass.getDeclaredMethod(
          "sendPacket",
          Reflection.nmsClass("Packet")
        );

      PACKET_SB_OBJ =
        Reflection
          .nmsClass("PacketPlayOutScoreboardObjective")
          .getConstructor();
      PACKET_SB_DISPLAY_OBJ =
        Reflection
          .nmsClass("PacketPlayOutScoreboardDisplayObjective")
          .getConstructor();
      PACKET_SB_SCORE =
        Reflection.nmsClass("PacketPlayOutScoreboardScore").getConstructor();
      PACKET_SB_TEAM =
        Reflection.nmsClass("PacketPlayOutScoreboardTeam").getConstructor();
      ...
      if (VersionType.V1_8.isHigherOrEqual()) {
        ...
        ENUM_SB_HEALTH_DISPLAY_INTEGER =
          Reflection.enumValueOf(ENUM_SB_HEALTH_DISPLAY, "INTEGER");
        ENUM_SB_ACTION_CHANGE =
          Reflection.enumValueOf(ENUM_SB_ACTION, "CHANGE");
        ENUM_SB_ACTION_REMOVE =
          Reflection.enumValueOf(ENUM_SB_ACTION, "REMOVE");
      } else {
  ...
  /**
   * Creates a new FridayBoard.
   *
   * @param player the player the scoreboard is for
   */
  public FridayBoard(Player player) {
   ...
   *
   * @return current player for this FridayBoard
   */
  ...
  private void checkLineNumber(int line, boolean checkMax) {
    ...
    if (checkMax && line >= lines.size()) {
      throw new IllegalArgumentException(
        "Line number must be under " + lines.size()
      );
    }
  }
  ...
  private void sendTeamPacket(int score, TeamMode mode)
    ...
    if (mode == TeamMode.CREATE || mode == TeamMode.UPDATE) {
      ...
      if (line == null || line.isEmpty()) {
        ...
        if (
          suffixTmp.length() >= 2 && suffixTmp.charAt(0) == ChatColor.COLOR_CHAR
        ) {
        ...
        suffix =
          (
            addColor
              ? (color.isEmpty() ? ChatColor.RESET.toString() : color)
              : ""
          ) +
          suffixTmp;
      }
      ...
      if (mode == TeamMode.CREATE) {
        setField(
          packet,
          Collection.class,
          Collections.singletonList(getColorCode(score))
        ); // Players in the team
      }
    }
  ...
  private void sendPacket(Object packet) throws ReflectiveOperationException {
    if (deleted) {
      throw new IllegalStateException("This FridayBoard is deleted");
    }
  ...
  private void setComponentField(Object object, String value, int count)
    throws ReflectiveOperationException {
    if (VERSION_TYPE != VersionType.V1_13) {
    ...
    for (Field f : object.getClass().getDeclaredFields()) {
      if (
        (f.getType() == String.class || f.getType() == CHAT_COMPONENT_CLASS) &&
        i++ == count
      ) {
        f.setAccessible(true);
