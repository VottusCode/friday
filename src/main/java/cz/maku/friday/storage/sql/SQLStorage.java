...
public class SQLStorage {
  ...
  public void createTable(Map<String, Class<?>> cols) {
    ...
    Bukkit
      ...
      .runTaskAsynchronously(
        ...
        () -> {
          try {
            PreparedStatement st = Friday
              .getPlugin()
              .getDataSource()
              .con.prepareStatement(
                "CREATE TABLE " + table + " " + finalColsString + ";"
              );
            st.executeUpdate();
            st.close();
            logger.log(
            ...
            if (!var10.getMessage().toLowerCase().contains("exists")) {
              ...
              logger.log(
                ...
                "There is an exception while creating table " +
                table +
                ". Check console.",
                false
              );
  ...
  public Map<String, String> selectTop(
    String where,
    String value,
    String finalKey,
    String top
  ...
  public void insert(Insert... builders) {
    ...
    try {
      PreparedStatement st = connection()
        .prepareStatement(sql.replace("'-1'", "NULL"));
      st.executeUpdate();
      st.close();
