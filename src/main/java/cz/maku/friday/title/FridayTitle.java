...
public class FridayTitle {
  ...
  public void send(TitleFormat format, FridayPlayer fp) {
    if (format.equals(TitleFormat.SUCCESS)) {
      fp
        .getPlayer()
        .sendTitle("§2§l" + title, "§a" + subtitle, fadeIn, stay, fadeOut);
    } else if (format.equals(TitleFormat.WARNING)) {
      fp
        .getPlayer()
        .sendTitle("§6§l" + title, "§e" + subtitle, fadeIn, stay, fadeOut);
    } else if (format.equals(TitleFormat.ERROR)) {
      fp
        .getPlayer()
        .sendTitle("§4§l" + title, "§c" + subtitle, fadeIn, stay, fadeOut);
    } else if (format.equals(TitleFormat.LOADING)) {
      fp
        .getPlayer()
        .sendTitle("§8§l" + title, "§7" + subtitle, fadeIn, stay, fadeOut);
